...
serve(async (req) => {
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
      model: "gemini-pro", // Updated to use the stable model version
      generationConfig: {
        temperature: 0.7,
        topK: 40,
        topP: 0.95,
    ...
    const prompt = `You are an assistant specialized in automating and editing contract templates. Your function is to process uploaded templates, locate the Contracting Parties clause, and automatically replace existing data with previously defined dynamic parameters.
    ...
    {
      "text": "the contract text with variables replaced",
      "variables": {
        "variable_name": "Field description in Portuguese"
      }
    }
